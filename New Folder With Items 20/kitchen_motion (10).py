...
def _get_current_time():
    """Get simulated or real time - automatic based on freeze status"""
    try:
        if _state("input_boolean.time_freeze_active") == "on":
            override = _state("input_datetime.time_freeze_clock")
            if override:
                try:
                    return datetime.fromisoformat(override.replace("T", " "))
                except Exception:
                    pass
    except Exception:
        pass
...
async def kitchen_motion_listener(**kwargs):
    ...
    new = _state(eid)
    _info(f"Listener: {eid} -> {new} @ {_get_current_time().strftime('%H:%M:%S')}")
